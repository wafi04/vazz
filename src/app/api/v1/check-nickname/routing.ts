...
import { getUrl, ResultFromCoda } from './utils'
import * as router from './router'

export default async function callAPI(request: NextRequest): Promise<ResultFromCoda> {
  const url = new URL(request.url)
  const searchParams = url.searchParams
  
  const game = searchParams.get('game')
  const id = searchParams.get('userId') || searchParams.get('id')
  const server = searchParams.get('serverId') || searchParams.get('zone') || searchParams.get('server')
