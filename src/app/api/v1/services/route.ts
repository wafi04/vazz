    ...
    for (const category of categories) {
      ...
      for (let i = 0; i < matchingItems.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            async (tx) => {
              for (const item of batch) {
                ...
                if (item.category === "Voucher" || item.category === "PLN") {
                  defaultProfits.isProfitFixed = "active";
                }
                ...
                if (existingService) {
                  ...
                  if (existingService.isProfitFixed) {
                    ...
                    resellerPrice = Math.round(
                      hargaModal +
                        (hargaModal * existingService.profitReseller) / 100
                    );
                    platinumPrice = Math.round(
                      hargaModal +
                        (hargaModal * existingService.profitPlatinum) / 100
                    );
                  }
                  ...
                  if (defaultProfits.isProfitFixed) {
                    ...
                    resellerPrice = Math.round(
                      hargaModal +
                        (hargaModal * defaultProfits.profitReseller) / 100
                    );
                    platinumPrice = Math.round(
                      hargaModal +
                        (hargaModal * defaultProfits.profitPlatinum) / 100
                    );
                  }
          ...
          console.error(
            `Batch processing error for category ${category.brand}:`,
            error
          );
          continue;
        }
